...
library IterableMapping {
    function insert(itmap storage self, uint key, uint price, address payable poster, address payable lowest_bidder, address payable next_lowest_bidder, string memory description, bool claimed, bool accepted, bool paid) internal returns (bool replaced) {
...
contract SupplyMesh {
    ...
    function acceptBidOnBounty(uint key) public returns (bool) {
        ...
        if (bountyEntry.claimed) {
            data.insert(max_key, bountyEntry.price, bountyEntry.poster, bountyEntry.lowest_bidder, bountyEntry.next_lowest_bidder, bountyEntry.description, bountyEntry.claimed, true, bountyEntry.paid);
            return true;
        } else return false;
    }
    ...
    function rejectBidOnBounty(uint key) public returns (bool) {
        IndexValue memory bountyEntry = data.iterate_get(key);
        if (bountyEntry.claimed) {
            data.insert(max_key, bountyEntry.price, bountyEntry.poster, bountyEntry.next_lowest_bidder, bountyEntry.next_lowest_bidder, bountyEntry.description, bountyEntry.claimed, false, bountyEntry.paid);
            return true;
        } else return false;
    }
    ...
    function addBountyForEntity(uint price, string memory description) public returns (bool) {
        data.insert(max_key, price, msg.sender, msg.sender, msg.sender, description, false, false, false);
        max_key++;
        return true;
    }
    ...
    function fundBountyForEntity(uint volume, uint key) public payable returns (bool) {
        ...
        if (bountyEntry.paid) {
            if (bountyEntry.balance + volume > bountyEntry.price) {
                ...
                data.insert(max_key, bountyEntry.price, bountyEntry.poster, bountyEntry.lowest_bidder, bountyEntry.next_lowest_bidder, bountyEntry.description, bountyEntry.claimed, bountyEntry.accepted, true);
                return true;
            } else {
